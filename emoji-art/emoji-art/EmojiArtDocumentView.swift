...
struct EmojiArtDocumentView: View {
    ...
    private func zoomGesture() -> some Gesture {
        MagnificationGesture()
            // where gestureZoomScale is an inout parameter
            .updating($gestureZoomScale) { latestGestureScale, gestureZoomScale, transaction in
                // in out will update gestureZoomScale because whilst this is in the view its own by the gesture (we will always let the gesture handle this), once its back it will hold the original
    ...
    // Doubletap to fit the entire image to the space available (will zoom to fit)
